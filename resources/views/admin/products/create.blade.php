      ...
      <form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          ...
          <div>
            ...
            <input type="text" id="category" name="category" value="{{ old('category') }}" placeholder="e.g., Sweaters, Jackets" required
        ...
        <div class="mb-6">
          ...
          <textarea id="description" name="description" rows="5" required
            class="w-full rounded-lg border border-gray-300 px-4 py-2 focus:border-blue-500 focus:outline-none @error('description') border-red-500 @enderror">{{ old('description') }}</textarea>
          @error('description')
          <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
          @enderror
        </div>
        ...
        <div class="mb-6 flex items-center">
          <input type="checkbox" id="in_stock" name="in_stock" value="1" {{ old('in_stock') ? 'checked' : '' }}
